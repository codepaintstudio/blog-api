...
<template>
  <div class="min-h-[calc(100vh-4rem)]">
    <div class="max-w-4xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
      <div
        class="bg-[var(--color-canvas-subtle)] shadow-2xl p-6"
      >
        <h1 class="text-3xl font-bold text-[var(--color-accent-emphasis)] mb-8">编辑帖子</h1>
        ...
        <form v-if="article" @submit.prevent="handleSubmit" class="space-y-6">
          <div>
            ...
            <input
              ...
              type="text"
              required
              class="mt-1 block w-full border bg-[var(--color-canvas-default)] border-[var(--color-accent-emphasis)] px-3 py-2 text-[var(--color-accent-emphasis)] focus:border-[#1f80ff] focus:outline-none focus:ring-1 focus:ring-[#1f80ff]"
            />
          </div>
          ...
          <div class="flex justify-end space-x-4">
            <router-link :to="'/article/' + route.params.id" class="btn-secondary">
              取消
            </router-link>
        ...
        <div v-else class="text-center py-12">
          <p class="text-[var(--color-accent-emphasis)]">加载中...</p>
        </div>
      </div>
    </div>
  </div>
